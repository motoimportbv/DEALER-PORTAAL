...
### P0 - Afgerond
- **Introductietarief €60 voor nieuwe /taxatie-klanten + standaard verzenden/uitprinten regels** (Feb 2026): Wanneer een dealer zich aanmeldt via `/taxatie` krijgt de klant `source="taxatie_aanvraag"`. Bij de eerste factuur wordt automatisch het introductietarief van **€60 ex BTW** voorgesteld (i.p.v. de nieuwe vaste prijs €120 ex BTW) met een blauwe banner "🎉 Introductietarief — eerste taxatie". Na het aanmaken van de factuur wordt `intro_used=true` gezet en valt de volgende factuur automatisch terug op €120. Op **elke** taxatie-factuur worden standaard 2 extra regels toegevoegd: **Verzendkosten €10 ex** + **Uitprinten/drukwerk €10 ex** (21% BTW), bewerkbaar via een eigen card "Extra Regels". Nieuwe velden op `taxatie_invoices`: `extra_lines: [{description, amount, btw_pct}]` + `is_intro: bool`. Nieuw endpoint `GET /api/customers/{id}/intro-pricing` checkt: `source=="taxatie_aanvraag"` AND `!intro_used` AND `invoice_count==0` → `is_eligible`. End-to-end getest via curl: intro detectie + intro_used flip + extra_lines persist + totaalberekening (€60 + €12.60 BTW + €10 + €10 + €4.20 BTW = €96.80).
