...
### P0 - Afgerond
- **Aangifte BPM — Pagina 1 & 6 bewerkbaar** (Feb 2026): Aparte knop "Aangifte BPM (bewerken)" in BPM Rapport view opent modal met alle bewerkbare velden van pagina 1 (VIN, document kenmerk 1c, BSN/RSIN) en pagina 6 (document kenmerk, vraag 9 radio + toelichting, vraag 9.2 radio, ondertekenaar, datum, 4 bijlage-checkboxes). Nieuwe endpoints `GET/POST /api/taxatie-programma/{id}/aangifte-overrides` + `/aangifte-bpm-pdf`. Overrides worden opgeslagen op de taxatie voor hergebruik. Andere pagina's blijven default ingevuld. Radio-button matching gebruikt PyMuPDF-decoded states (#20 → spatie); niet-matchende widgets blijven Off. Getest met curl + PyMuPDF read-back.
