...
### P0 - Afgerond
- **Maandelijks taxateur-stats rapport** (Feb 2026): Op de 1e van elke maand om ~09:00 NL-tijd (08:00 UTC) wordt automatisch 1 e-mail verstuurd naar `motoimportbv@gmail.com` met per taxateur (rol="taxateur" in `users`) twee tellers over de **vorige kalendermaand**: (1) aantal `taxatie_aanvragen` met status="afgerond" door die taxateur, (2) aantal `taxatie_invoices` aangemaakt door die taxateur. Sortering: meeste rapporten eerst. Service: `backend/services/taxateur_stats.py`. Background-task: `monthly_taxateur_report_task` in `server.py` (idempotent via `system_tasks` markers). Manueel triggeren: `POST /api/admin/taxateur-monthly-report/send-now` (body `{"dry_run":true}` voor preview zonder mail). **Audit-trail toegevoegd** aan `update_status` endpoint: `completed_at`, `completed_by_id`, `completed_by_email` worden geregistreerd zodra een aanvraag op "afgerond" wordt gezet — dit is de bron voor de aanvragen-teller. End-to-end getest: dry-run + echte verzending bevestigd in logs.
