...
### P0 - Afgerond
- **Auto-specifieke schade-checklist + brand biases** (Feb 2026): Naast de motorlijst nu een aparte 33-item auto-lijst (Bumper, Voorruit, Distributieriem, Airco, Schokdempers, Velgen, etc.). 28 auto-merken (VW, Audi, BMW, Mercedes, Toyota, Tesla, etc.) met merk-specifieke zwakheden in `BRAND_BIASES_AUTO`. Frontend kiest automatisch op basis van `user.vehicle_type` welke lijst geladen wordt.
